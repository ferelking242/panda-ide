...
/// Model features & capabilities descriptor
class ModelCapabilities {
  final bool hasThinking;
  final bool hasVision;
  final bool hasTools;
  final bool hasSearch;
  final String contextWindowStr;
  final double inputCost; // USD per 1M tokens
  final double outputCost; // USD per 1M tokens
  ...
  const ModelCapabilities({
    ...
    required this.contextWindowStr,
    required this.inputCost,
    required this.outputCost,
  });

  static ModelCapabilities of(String modelId) {
    ...
    if (m.contains('gemini-2.5-pro') || m.contains('gemini-1.5-pro')) {
      ...
      inCost = 1.25;
      outCost = 10.00;
    } else if (m.contains('gemini-2.5-flash-lite') || m.contains('gemini-2.0-flash-lite') || m.contains('gemini-1.5-flash-8b')) {
      ctx = '1M';
      inCost = 0.075;
      outCost = 0.30;
    } else if (m.contains('gemini-2.5-flash') || m.contains('gemini-2.0-flash') || m.contains('gemini-1.5-flash') || m.contains('gemini-3')) {
      ctx = '1M';
      inCost = 0.15;
      outCost = 0.60;

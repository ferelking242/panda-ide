...
class MainActivity : FlutterActivity() {
        ...
        updateChannel.setMethodCallHandler { call, result ->
            when (call.method) {
                "downloadAndInstallApk" -> {
                    ...
                    Thread {
                        try {
                            val apk = downloadUpdateApk(url, filename) { progress, bytes, total ->
                                runOnUiThread {
                                    updateChannel.invokeMethod("onProgress", mapOf(
            ...
            connection.inputStream.use { input ->
                FileOutputStream(target, isPartial).use { output ->
                    while (true) {
                        ...
                        val progress = if (totalBytes > 0) copiedBytes.toDouble() / totalBytes else -1.0
    ...
    private fun syncDirectoryToTree(localDir: File, targetRoot: DocumentFile) {
        ...
        localDir.listFiles()?.forEach { child ->
            if (child.name == ".panda_source.json" || child.name?.endsWith(".panda_source.json") == true) {
                return@forEach
            }
    ...
    private fun importUriToAppFile(uri: Uri, progress: ProgressDialogHandle, title: String): String? {
        ...
        return try {
            ...
            writeSourceMetadata(
                File("${targetFile.absolutePath}.panda_source.json"),
                uri.toString(),
                sanitizedName,
                "file",
